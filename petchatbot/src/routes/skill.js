...
router.post('/start', (req, res) => {
  ...
  const pet = petManager.initRoom(roomId, userId);
...
router.post('/info', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);

  const status = petManager.getPetStatus(roomId, BASE_URL);
  if (!status) {
  ...
  const title = achievements.getActivePetTitle(pet.pet_id, roomId);
  ...
  // 업적 체크
  achievements.checkAndGrant(roomId, pet, userId);
...
router.post('/feed', (req, res) => {
  ...
  if (foodId) {
    return doFeed(res, roomId, userId, foodId);
  }

  // 아니면 먹이 목록 보여주기
  const foods = petManager.getFoodList();
  const user = petManager.getUserInfo(userId, roomId);
...
router.post('/feed/:foodId', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  doFeed(res, roomId, userId, req.params.foodId);
});

function doFeed(res, roomId, userId, foodId) {
  const result = petManager.feedPet(roomId, userId, foodId);
  ...
  // 황금 알 이벤트 체크
  const goldenEgg = events.tryGoldenEgg(roomId, userId);
  if (goldenEgg) {
    message += `\n\n${goldenEgg.message}`;
  }
...
router.post('/train', (req, res) => {
  ...
  if (statType) {
    return doTrain(res, roomId, userId, statType);
  }
...
router.post('/train/:statType', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  doTrain(res, roomId, userId, req.params.statType);
});

function doTrain(res, roomId, userId, statType) {
  const result = petManager.trainPet(roomId, userId, statType);
...
router.post('/rename', (req, res) => {
  ...
  const result = petManager.renamePet(roomId, newName);
...
router.post('/raid', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const result = events.startRaid(roomId, userId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/defend', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const result = events.defendRaid(roomId, userId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/ranking', (req, res) => {
  ...
  const rankings = events.getRanking(10);
...
router.post('/contribution', (req, res) => {
  ...
  const rankings = events.getRoomRanking(roomId);
  if (rankings.length === 0) {
...
router.post('/expedition', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const result = expedition.startExpedition(roomId, userId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/return', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const result = expedition.collectExpedition(roomId, userId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/diary', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const text = diary.generateDiary(roomId);
  res.json(kakao.simpleText(text));
});
...
router.post('/fortune', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const result = fortune.drawFortune(userId, roomId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/rps', (req, res) => {
  ...
  const result = fortune.rockPaperScissors(userId, roomId, choice, bet);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/titles', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const pet = petManager.getPetStatus(roomId, BASE_URL)?.pet;
  if (!pet) return res.json(kakao.simpleText('펫이 없어요!'));

  const text = achievements.getAllAchievements(pet.pet_id, roomId, userId);
  res.json(kakao.simpleText(text));
});

// ── /심부름 [방코드] ─────────────────────────────────

router.post('/errand', (req, res) => {
  const { userId, roomId, params } = extractContext(req.body);
  petManager.initRoom(roomId, userId);

  const targetCode = params.room_code;
  if (!targetCode) {
    const myCode = errand.getRoomCode(roomId);
  ...
  const pet = petManager.getPetStatus(roomId, BASE_URL)?.pet;
  const result = errand.sendErrand(roomId, targetCode, pet?.name || '펫');
  res.json(kakao.simpleText(result.message));
});
...
router.post('/roomcode', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const code = errand.getRoomCode(roomId);
...
router.post('/quiz', (req, res) => {
  const { userId, roomId } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const result = quiz.createQuiz(roomId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/answer', (req, res) => {
  ...
  const result = quiz.answerQuiz(roomId, userId, answer);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/deco', (req, res) => {
  ...
  // 보유 데코 표시
  const owned = deco.getRoomDecos(roomId);
  const shop = deco.getDecoShop();
...
router.post('/deco/buy', (req, res) => {
  const { userId, roomId, params } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const decoId = params.deco_id;
  if (!decoId) return res.json(kakao.simpleText('사용법: /데코구매 [아이템ID]\n예: /데코구매 red_sofa'));
  const result = deco.buyDeco(userId, roomId, decoId);
  res.json(kakao.simpleText(result.message));
});

// ── /장착 [deco_id] ──────────────────────────────────

router.post('/deco/equip', (req, res) => {
  const { userId, roomId, params } = extractContext(req.body);
  petManager.initRoom(roomId, userId);
  const decoId = params.deco_id;
  if (!decoId) return res.json(kakao.simpleText('사용법: /장착 [아이템ID]'));
  const result = deco.equipDeco(roomId, decoId);
  res.json(kakao.simpleText(result.message));
});
...
router.post('/rebirth', (req, res) => {
  ...
  if (confirm === '확인') {
    const result = rebirth.doRebirth(roomId);
    return res.json(kakao.simpleText(result.message));
  }

  const check = rebirth.canRebirth(roomId);
...
router.post('/fallback', (req, res) => {
  ...
  if (lastActive !== now) {
    events.giveGold(userId, roomId, 20, '일일 출석 보상');
  }
  ...
  switch (cmd) {
    ...
    case '/상점': {
      const foods2 = petManager.getFoodList();
      const user2 = petManager.getUserInfo(userId, roomId);
      return res.json(kakao.textWithQuickReplies(
    ...
    case '/내정보': {
      const u = petManager.getUserInfo(userId, roomId);
    ...
    case '/약탈':
      return res.json(kakao.simpleText(events.startRaid(roomId, userId).message));
    case '/방어':
      return res.json(kakao.simpleText(events.defendRaid(roomId, userId).message));
    case '/랭킹': {
      const rankings = events.getRanking(10);
    ...
    case '/기여도': {
      const cr = events.getRoomRanking(roomId);
    ...
    case '/먹이':
      if (arg) return doFeed(res, roomId, userId, arg);
    ...
    case '/훈련':
      if (arg) return doTrain(res, roomId, userId, arg);
      return res.json(kakao.textWithQuickReplies(
        '⚔️ 어떤 훈련을 할까요? (비용: 15G)',
        [
          { label: '💪 근력', messageText: '/훈련 strength' },
    ...
    case '/이름':
      ...
      const renameResult = petManager.renamePet(roomId, arg);
      return res.json(kakao.simpleText(renameResult.message));

    case '/탐험':
      return res.json(kakao.simpleText(expedition.startExpedition(roomId, userId).message));
    case '/귀환':
      return res.json(kakao.simpleText(expedition.collectExpedition(roomId, userId).message));
    case '/일기':
      return res.json(kakao.simpleText(diary.generateDiary(roomId)));
    case '/운세':
      return res.json(kakao.simpleText(fortune.drawFortune(userId, roomId).message));
    case '/가위바위보': {
      ...
      return res.json(kakao.simpleText(fortune.rockPaperScissors(userId, roomId, choice, bet).message));
    }
    case '/퀴즈':
      return res.json(kakao.simpleText(quiz.createQuiz(roomId).message));
    case '/정답':
      if (!arg) return res.json(kakao.simpleText('사용법: /정답 [답]'));
      return res.json(kakao.simpleText(quiz.answerQuiz(roomId, userId, arg).message));
    case '/심부름':
      if (!arg) {
        const myCode = errand.getRoomCode(roomId);
        return res.json(kakao.simpleText(`🏃 심부름\n우리 방 코드: ${myCode}\n사용법: /심부름 [상대방 코드]`));
      }
      return res.json(kakao.simpleText(errand.sendErrand(roomId, arg, petManager.getPetStatus(roomId, BASE_URL)?.pet?.name || '펫').message));
    case '/방코드':
      return res.json(kakao.simpleText(`🔑 우리 방 코드: ${errand.getRoomCode(roomId)}`));
    case '/데코': {
      const user3 = petManager.getUserInfo(userId, roomId);
      const owned = deco.getRoomDecos(roomId);
      const shop = deco.getDecoShop();
    ...
    case '/데코구매':
      ...
      return res.json(kakao.simpleText(deco.buyDeco(userId, roomId, arg).message));
    case '/장착':
      if (!arg) return res.json(kakao.simpleText('사용법: /장착 [아이템ID]'));
      return res.json(kakao.simpleText(deco.equipDeco(roomId, arg).message));
    case '/환생':
      if (arg === '확인') return res.json(kakao.simpleText(rebirth.doRebirth(roomId).message));
      const check = rebirth.canRebirth(roomId);
    ...
    case '/칭호': {
      const pet = petManager.getPetStatus(roomId, BASE_URL)?.pet;
      if (!pet) return res.json(kakao.simpleText('펫이 없어요!'));
      return res.json(kakao.simpleText(achievements.getAllAchievements(pet.pet_id, roomId, userId)));
    }
